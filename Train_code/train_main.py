...
def train_main(classification_type, balanced, preprocessing_method, threshold, names_classifiers):
	train_data_dir =  "C:/Users/it/Desktop/imagingFlowCytometry-master-1/imagingFlowCytometry-master/Stained_CP_Features/Stained_CP_Features"

	print("Train data directory:", train_data_dir)
	print("Does the directory exist?", os.path.exists(train_data_dir))
	...
	if preprocessing_method == '':
		...
		preprocessing_method = '_' + preprocessing_method
	if preprocessing_method == 'low_var':
		output_dir = 'Classifiers/' + classification_type.capitalize() + '_' + balanced + preprocessing_method + '_' + str(threshold)
	else:
		output_dir = '/Classifiers/' + classification_type.capitalize() + '_' + balanced + preprocessing_method
	if not os.path.exists(output_dir):
		os.makedirs(output_dir)
		
	#Get the intersection of the zero score features over all subjects
	if preprocessing_method == 'remove_zero_features':
		if classification_type == 'all':
	...
	if preprocessing_method == 'top_10_features':
		if classification_type == 'all':
	...
	# dimensionality reduction: PCA or LDA or autoencoder
	scaler = preprocessing.StandardScaler()  # standardize features
	train_data = pandas.DataFrame(scaler.fit_transform(train_data), columns=train_data.columns)

	# Apply PCA if preprocessing_method == 'pca'
	if preprocessing_method == 'pca':
		pca = PCA(n_components=min(2, train_data.shape[1]))  # set 2 components
		train_data = pca.fit_transform(train_data)
		print(f"Explained variance ratio: {pca.explained_variance_ratio_}")

	# Apply LDA if selected
	if preprocessing_method == 'lda':
		lda = LDA(n_components=min(2, train_data.shape[1]))  # Choose the number of components
		train_data = lda.fit_transform(train_data, ground_truth)
		print(f"LDA explained variance ratio: {lda.explained_variance_ratio_}")

	# Apply Autoencoder if selected
	if preprocessing_method == 'autoencoder':
		input_dim = train_data.shape[1]
		encoding_dim = min(10, input_dim)  # Set the encoding dimension
		...
		# Get the encoded data (latent representation)
		encoder = Model(input_layer, encoded)
		train_data = encoder.predict(train_data)

		print(f"Autoencoder compressed data shape: {train_data.shape}")
