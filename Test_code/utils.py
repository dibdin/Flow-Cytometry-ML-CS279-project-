...
def write_wbc_count_to_file(wbc_count, output_dir):
	...
	pd.DataFrame(wbc_count.items()).to_csv(''.join(output_dir + '/' + wbc_count_file_name))
...
def plot_wbc_count(wbc_count_patient, wbc_counts_ref, comp_to, cell_types, classifier, output_dir):
	...
	plt.bar(y_pos, wbc_count_patient/(numpy.sum(wbc_count_patient)/100.), bar_width, alpha=opacity, align='center', color='red', label='Patient')
